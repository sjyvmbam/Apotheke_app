...
public class DaoConnectionImp extends AbstractDaoConnection implements DaoConnection {
    ...
    public void addNewMedikament(Medikament medikament){
        ...
        try {
            conn = getConnection();
            //String query = "insert into medicament(pharmazentralnummer,stueckzahl)" + "VALUES("+pharmazentralnummer +"," +stueckzahl+")";
            String query="insert into medicament (product_name, pharmazentralnummer, stueckzahl) select "+name+", "+pharmazentralnummer+", "+stueckzahl+" where not exists( select 1 from medicament where pharmazentralnummer = ?)";
            assert conn != null;
            statement = conn.prepareStatement(query);
            statement.setInt(1,pharmazentralnummer);
            statement.executeUpdate();
    ...
    public void reduktieren(Medikament medikament) {
        ...
        try {
            conn = getConnection();
            String query = "select id,pharmazentralnummer,stueckzahl from medicament where pharmazentralnummer=?";
            assert conn != null;
            statement = conn.prepareStatement(query);
            ...
            while(result.next()){
                ...
                Collections.addAll(listMedikament,pharmazentralnummer,stueckzahl);
                    ...
                    String update="update medicament set stueckzahl=?  where (pharmazentralnummer=?)";
                    String insertQuery="insert into verkauft_medikament (produktverkauft, medikament_id) select "+stueck+", "+Id+" where exists( select 1 from medicament where pharmazentralnummer = ?)";
                    assert conn != null;

                    statement= conn.prepareStatement(update);
                    statement.setInt(1,stueckzahl);
                    statement.setInt(2,referenz);
                    statement.executeUpdate();
    ...
    public void erhoehen(Medikament medikament) {
        ...
        try {
            conn = getConnection();
            String query = "select pharmazentralnummer,stueckzahl from medicament";
            assert conn != null;
            statement = conn.prepareStatement(query);
            ResultSet result= statement.executeQuery();
            List<Integer> listMedikament= new ArrayList<>();
            int pharmazentralnummer;
            int  stueckzahl;
            while(result.next()){
                pharmazentralnummer=result.getInt("pharmazentralnummer");
    ...
    public List<Medikament> findAll() {
        ...
        try {
            ...
            while(result.next()){
                ...
                int pharmazentralnummer=result.getInt("pharmazentralnummer");
                int  stueckzahl= result.getInt("stueckzahl");
                Medikament medikament= new Medikament(id,pharmazentralnummer,stueckzahl);
    ...
    @Override
    public Medikament findOne(int referenz) {
        ...
        try {
            ...
            while(result.next()){
                ...
                int pharmazentralnummer=result.getInt("pharmazentralnummer");
                int  stueckzahl= result.getInt("stueckzahl");
                 medikament= new Medikament(id,pharmazentralnummer,stueckzahl);
            }
